...
// Strictly validate shop domain like "mystore.myshopify.com"
...
module.exports = async (req, res) => {
  try {
    ...
    if (!shop) {
      res.statusCode = 400;
      return res.end("Missing ?shop=my-shop.myshopify.com");
    }
    if (!isValidShop(shop)) {
      res.statusCode = 400;
    ...
    // 1) Generate per-request, URL-safe state
    const state = crypto.randomBytes(16).toString("base64url");

    // 2) Set HttpOnly cookie so callback can validate state
    // Key by shop to avoid clashes across multiple tabs/shops
    ...
    // 3) Redirect to Shopify OAuth with the SAME state
    const params = new URLSearchParams({
      client_id: process.env.SHOPIFY_API_KEY,
      scope: process.env.SHOPIFY_SCOPES,
      redirect_uri: process.env.SHOPIFY_REDIRECT_URI, // e.g. https://<your-domain>/api/callback
      state
    });
